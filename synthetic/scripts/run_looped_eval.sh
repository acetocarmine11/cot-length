...
for model_size in "${model_size[@]}"; do
    for test_T in "${test_Ts[@]}"; do
        for test_t in "${test_values[@]}"; do
            ...
            python3 eval.py --t=4 --test_t=$test_t --model_size=$model_size --T=32 --test_T=$test_T --device=1 --caption="looped_model" --model_type=looped_gpt2 --n_loop=2
        done
    done
done
